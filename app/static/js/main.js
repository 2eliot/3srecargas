...
(function () {
    ...
    var supportNameInput = document.getElementById('supportName');
    var supportFirstMessageInput = document.getElementById('supportFirstMessage');
    var supportWebsiteInput = document.getElementById('supportWebsite');
    var supportErrorEl = document.getElementById('supportError');
    var supportChatEl = document.getElementById('supportChat');
    var supportThreadEl = document.getElementById('supportThread');
    var supportChatCodeEl = document.getElementById('supportChatCode');
    var supportChatStatusEl = document.getElementById('supportChatStatus');
    var supportReplyForm = document.getElementById('supportReplyForm');
    var supportReplyBody = document.getElementById('supportReplyBody');
    var supportFloatDot = document.getElementById('supportFloatDot');
    var supportAttachBtn = document.getElementById('supportAttachBtn');
    ...
    // --- Chat de soporte -------------------------------------------------
    //
    // Para abrir un chat el cliente solo escribe su nombre. Todo lo demas
    // se recoge de la propia pagina: si escribe desde el estado de su
    // pedido, el numero de orden viaja solo y en el panel del admin llega
    // ya identificado sin haber tecleado nada mas.
    //
    // El hilo se sostiene con sondeo HTTP y no con WebSocket porque el
    // servidor corre `gunicorn -w 3` sin estado compartido: el worker que
    // recibe la respuesta del admin no seria el mismo que sostiene el
    // socket del cliente.

    var SUPPORT_TOKEN_KEY = 'store:support-token';
    var supportState = { token: '', lastId: 0, timer: null, sending: false };

    function supportReadToken() {
        try { return localStorage.getItem(SUPPORT_TOKEN_KEY) || ''; } catch (_) { return ''; }
    }

    function supportWriteToken(token) {
        supportState.token = token || '';
        try {
    ...
    function supportHeaders(extra) {
        var headers = extra || {};
        // La cookie httponly es el camino normal; la cabecera es el
        // respaldo para navegadores que la borran por su cuenta (Safari
        // purga cookies de sitios que no se visitan seguido, y perder la
        // cookie seria perder el hilo de alguien esperando respuesta).
        if (supportState.token) headers['X-Support-Token'] = supportState.token;
        return headers;
    }

    function supportShowError(message) {
        if (!supportErrorEl) return;
        supportErrorEl.textContent = message || '';
        supportErrorEl.hidden = !message;
    }

    function supportCollectContext() {
        var context = { page: location.pathname.slice(0, 255) };

        // Numero de orden, si el chat se abre desde la pagina del pedido.
        // La variable la fija order_status.html; la URL (/order/<numero>) es
        // el respaldo por si esa plantilla cambia de nombre de variable.
        if (window.CURRENT_ORDER_NUMBER) {
            context.order_number = String(window.CURRENT_ORDER_NUMBER);
        } else {
            var orderMatch = location.pathname.match(/^\/order\/([A-Za-z0-9_-]+)/);
            if (orderMatch) context.order_number = orderMatch[1];
        }

        // Contacto que el cliente guardo en un checkout anterior.
        var remembered = getRememberedContact();
        if (remembered) {
            if (remembered.email) context.email = remembered.email;
            if (remembered.phone) context.phone = remembered.phone;
        }

        // El ID de jugador que ya tenga escrito en la pagina: es la
        // pista mas util de todas, porque cruza directo contra las
        // ordenes sin que el cliente tenga que dictar nada.
        var playerIdInput = document.getElementById('playerId');
        if (playerIdInput && playerIdInput.value) {
            context.player_id = String(playerIdInput.value).trim().slice(0, 100);
        }

        if (selectedPackage && selectedPackage.name) context.package = String(selectedPackage.name);
        if (currentGame && currentGame.name) context.game = String(currentGame.name);

        return context;
    }

    function supportTimeLabel(iso) {
        var d = new Date(iso);
        if (isNaN(d)) return '';
        return d.toLocaleTimeString('es-VE', { hour: '2-digit', minute: '2-digit' });
    }

    // textContent y nunca innerHTML: el cuerpo de un mensaje lo escribe una
    // persona y llega por JSON, sin pasar por el autoescape de Jinja.
    function supportRenderMessage(message) {
        if (!supportThreadEl) return;

        var bubble = document.createElement('div');
        bubble.className = 'support-bubble ' + (message.sender || 'admin');
        if (message.body) bubble.textContent = message.body;

        if (message.attachment) {
            var img = document.createElement('img');
            img.src = '/static/uploads/' + message.attachment;
        ...
        if (message.sender !== 'system') {
            ...
            time.textContent = supportTimeLabel(message.created_at);
            bubble.appendChild(time);
        }

        supportThreadEl.appendChild(bubble);
        supportState.lastId = Math.max(supportState.lastId, message.id || 0);
        supportThreadEl.scrollTop = supportThreadEl.scrollHeight;
    }

    function supportApplyChat(chat) {
        if (!chat) return;
        if (supportChatCodeEl) supportChatCodeEl.textContent = chat.code || '';
        if (supportChatStatusEl) supportChatStatusEl.textContent = chat.status_label || '';
    }

    function supportEnterChatView() {
        if (supportFloatDot) supportFloatDot.hidden = true;
        if (supportForm) supportForm.hidden = true;
        if (supportChatEl) supportChatEl.hidden = false;
    ...
    function supportPoll() {
        ...
        fetch('/soporte/hilo?after_id=' + supportState.lastId, {
            ...
            .then(function (data) {
                ...
                supportApplyChat(data.chat);
                (data.messages || []).forEach(supportRenderMessage);
            })
            .catch(function () {});
    }

    function supportStartPolling() {
        if (supportState.timer) return;
        // 5 s con el modal abierto y a la vista; en segundo plano no se
        // pregunta nada, que es donde se van las peticiones inutiles.
        supportState.timer = setInterval(function () {
    ...
    function supportOpenChat(payload) {
        supportWriteToken(payload.token);
        supportState.lastId = 0;
        if (supportThreadEl) supportThreadEl.innerHTML = '';
        (payload.messages || []).forEach(supportRenderMessage);
        supportApplyChat(payload.chat);
        supportEnterChatView();
        supportStartPolling();
    }
    ...
    if (supportForm) {
        supportForm.addEventListener('submit', function (evt) {
            ...
            var name = supportNameInput ? String(supportNameInput.value || '').trim() : '';
            if (name.length < 2) {
                supportShowError('Escribe tu nombre para iniciar el chat.');
                return;
            }

            var submitBtn = supportForm.querySelector('.support-submit-btn');
            ...
            fetch('/soporte/iniciar', {
                ...
                body: JSON.stringify({
                    name: name,
                    message: supportFirstMessageInput ? supportFirstMessageInput.value : '',
                    website: supportWebsiteInput ? supportWebsiteInput.value : '',
                    context: supportCollectContext()
                })
            })
                .then(function (r) { return r.json(); })
                .then(function (data) {
                    ...
                    if (!data || !data.ok) {
                        supportShowError((data && data.error) || 'No pudimos abrir el chat. Intenta de nuevo.');
                        return;
                    }
                    if (supportFirstMessageInput) supportFirstMessageInput.value = '';
                    supportOpenChat(data);
                })
                .catch(function () {
                    if (submitBtn) submitBtn.disabled = false;
                    supportShowError('Sin conexion. Revisa tu internet e intenta de nuevo.');
                });
        });
    }

    if (supportReplyForm) {
        supportReplyForm.addEventListener('submit', function (evt) {
            evt.preventDefault();
            if (supportState.sending) return;

            var body = supportReplyBody ? String(supportReplyBody.value || '').trim() : '';
            if (!body) return;

            supportState.sending = true;
            if (supportReplyBody) supportReplyBody.value = '';
            ...
            fetch('/soporte/mensaje', {
                ...
                .then(function (data) {
                    ...
                    if (data && data.ok) {
                        supportRenderMessage(data.message);
                        supportApplyChat(data.chat);
                    } else if (supportReplyBody) {
                        // Devolver el texto al cuadro: perder lo que alguien
                        // acaba de escribir por un error de red es la peor
                        // forma de recibir a quien ya viene molesto.
                        supportReplyBody.value = body;
                        alert((data && data.error) || 'No se pudo enviar el mensaje.');
                    }
                })
                .catch(function () {
                    supportState.sending = false;
                    if (supportReplyBody) supportReplyBody.value = body;
                });
        });
    }

    // Adjuntar una captura. Es el caso central del soporte de una tienda de
    // recargas: quien reclama un pago casi siempre tiene el comprobante en
    // la galeria y describirlo por escrito no lleva a ningun lado.
    if (supportAttachBtn && supportFileInput) {
        ...
        supportFileInput.addEventListener('change', function () {
            var file = supportFileInput.files && supportFileInput.files[0];
            if (!file) return;

            // El servidor tiene MAX_CONTENT_LENGTH en 16 MB, pero cortar
            // aqui evita subir una foto de 12 MB por una conexion movil
            // para que el servidor la rechace al final.
            if (file.size > 8 * 1024 * 1024) {
                alert('La imagen es muy pesada. Envia una de menos de 8 MB.');
            ...
            payload.append('body', supportReplyBody ? supportReplyBody.value : '');

            supportAttachBtn.disabled = true;
            fetch('/soporte/adjunto', {
                ...
                .then(function (r) { return r.json(); })
                .then(function (data) {
                    supportAttachBtn.disabled = false;
                    supportFileInput.value = '';
                    if (data && data.ok) {
                        if (supportReplyBody) supportReplyBody.value = '';
                        supportRenderMessage(data.message);
    ...
    if (supportReplyBody) {
        supportReplyBody.addEventListener('keydown', function (evt) {
            if (evt.key === 'Enter' && !evt.shiftKey) {
                evt.preventDefault();
                if (supportReplyForm) supportReplyForm.requestSubmit();
            }
        });
    }

    // Si este navegador ya tiene un hilo, se retoma donde quedo en vez de
    // arrancar otro.
    supportState.token = supportReadToken();
    if (supportState.token && supportModal) {
        fetch('/soporte/hilo?after_id=0', {
            ...
            .then(function (data) {
                if (!data || !data.ok) { supportWriteToken(''); return; }
                supportOpenChat({
                    token: supportState.token,
                    chat: data.chat,
                    messages: data.messages
                });
                // El modal está cerrado en este punto: el punto verde en el
                // botón flotante es lo único que le dice al cliente que
                // tiene una conversación viva esperándolo.
                if (supportFloatDot && data.chat && data.chat.status !== 'closed') {
                    supportFloatDot.hidden = false;
                }
            })
            .catch(function () {});
    }
